...
spec:
  ...
  rewritePath: /v1/                        # maps gateway paths onto vLLM's /v1/... routes
EOF
...
Get the gateway's external IP (the MetalLB-assigned one), then call Modelplane's endpoint. A `ModelService` is supposed to receive requests at `/<namespace>/<service>/v1/...`.

In this local kind setup, the composed route strips the wrong prefix and produces a 404 if you include `v1` in the URL. This is likely an issue because `ModelEndpoint` is hand-created rather than generated by Modelplane for a real `ModelDeployment`. 

The model test curl call should be `http://localhost:8080/ml-team/qwen/v1/chat/completions` but for a workaround here we drop `v1`.

```bash
curl -v http://localhost:8080/ml-team/qwen/chat/completions \
  -H "Content-Type: application/json" \
